...
> **IMPORTANTE:** Toda chamada `task()` neste documento DEVE incluir as instruções de State Protocol + Trello sync ao final do prompt. Use o template definido no `harness-orchestrator.agent.md` — seção "Task Template". O texto "(+ Trello sync)" marca cada task que deve recebê-la.
...
2. Invocar po-agent via task (+ Trello sync) para:
   ...
   - Produzir `.planning/PRD.md` com in/out scope, métricas, riscos
...
5. Transição 1→2:
    ".planning/PRD.md validado. Avançar para o Gate 2 (Plan)?"
   Opções: Avançar | Revisar PRD | Abortar
```

**Artefatos:** `.planning/PRD.md`, backlog estruturado

### Gate 2 — Plan

**Objetivo:** Desenhar arquitetura, quebrar tarefas, produzir `.planning/PLAN.md`.
...
2. Invocar techlead via task (+ Trello sync) para:
   ...
   - Arquitetura backend (Clean Architecture, DTOs, boundaries)
    - Diagramas Mermaid em `.planning/arch/`
...
5. Consolidar `.planning/PLAN.md`: o que será construído por camada, ordem, subagentes
6. Transição 2→3:
    ".planning/PLAN.md pronto. Avançar para o Gate 3 (Execute)?"
   Opções: Avançar | Revisar plano | Abortar
```

**Artefatos:** `.planning/arch/*.md`, `.planning/PLAN.md`
...
2. Analisar `.planning/PLAN.md` e determinar ordem:
...
4. Consolidar `.planning/SUMMARY.md` com resultados e desvios
5. Transição 3→4:
   "Implementação concluída. Avançar para o Gate 4 (Validate)?"
   Opções: Avançar | Revisar implementação | Abortar
```

**Artefatos:** Código implementado, commits, `.planning/SUMMARY.md`
...
**Artefatos:** Relatórios de review, `.planning/VALIDATION.md`, commits, Pull Request, CI report
...
2. Invocar po-agent via task (+ Trello sync) para:
   - Discovery completo (problema, personas, concorrência)
    - `.planning/PRD.md` do projeto (visão, escopo, métricas, riscos)
...
**Artefatos:** `.planning/PRD.md`, roadmap de funcionalidades
...
**Artefatos:** Projeto rodando, CI/CD configurado, `.planning/arch/`
...
3. Consolidar `.planning/SUMMARY.md` com a correção
4. Transição 2→3:
   "Código corrigido. Avançar para verificação?"
   Opções: Avançar | Revisar correção | Abortar
```

**Artefatos:** Código corrigido, commits, `.planning/SUMMARY.md`
...
**Artefatos:** Bug fechado, testes verdes, `.planning/VALIDATION.md`, commits, Pull Request, CI report
...
|-------|-----------|------|-----------|-------|
| feature | 1→2 | Pre-flight | `.planning/PRD.md` existe, requirements ok | Bloquear |
| feature | 2→3 | Pre-flight | `.planning/PLAN.md` existe, arquitetura revisada | Bloquear |
| feature | 3→4 | Pre-flight | Código commitado, `.planning/SUMMARY.md` | Bloquear |
| feature | 4→Done | Revision | Testes verdes, reviews ok, lint limpo | Loop max 3 |
| project | 1→2 | Pre-flight | `.planning/PRD.md` do projeto validado | Bloquear |
...
6. **Delegar sem contexto** — sempre passar `.planning/PRD.md` e `.planning/PLAN.md` ao agente antes de invocar
