...
Fallback:
  Se executado via hook e não houver contexto rico (PRD/PLAN/SUMMARY/REVIEW),
...
def generate_handoff(repo: Path, git_info: dict[str, Any], context: dict[str, Any], args) -> str:
    ...
    # contexto injetado (PRD/PLAN/SUMMARY/REVIEW) se houver
    prd = context.get("PRD.md") or context.get("prd") or ""
    plan = context.get("PLAN.md") or context.get("plan") or ""
    summary = context.get("SUMMARY.md") or context.get("summary") or ""
    review = context.get("REVIEW.md") or context.get("review") or ""
    ...
    if review:
        ...
        lines.append("- Decisões conforme PLAN e SUMMARY (ver git diff e commits).")
        lines.append("- Guard rails via `hooks/guard_rails.py` (determinístico) — HIGH deve estar zerado.")
        lines.append("")
    lines.append("## Pendências / Próximos passos")
    lines.append("")
    lines.append("- [ ] CI deve estar verde (ver `gh pr checks` / `gh run list`)")
    lines.append("- [ ] Se hook gerou HANDOFF minimal, agente shipper minimal pode complementar com contexto qualitativo (PRD/PLAN/REVIEW em memória)")
    ...
    lines.append("- `SUMMARY.md` — memória (builder)")
    lines.append("- `REVIEW.md` — memória (reviewer arquitetura apenas)")
    lines.append("")
    lines.append("---")
...
def generate_state(repo: Path, git_info: dict[str, Any], context: dict[str, Any], args, pr_url: str | None) -> str:
    ...
    lines.append("- `SUMMARY.md`: done (memória)")
    lines.append("- `REVIEW.md`: done (memória) — arquitetura apenas")
...
def main():
    ...
    parser.add_argument("--context-file", default="", help="Arquivo JSON com PRD/PLAN/SUMMARY/REVIEW em memória (opcional)")
