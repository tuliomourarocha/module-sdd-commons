...
O que faz (determinístico, sem LLM):
  ...
  - Em fail: coleta logs via `gh run view <id> --log-failed` ou `gh run view --json jobs` + `gh pr checks`, gera
    `.planning/CI_REPORT.md` e `.planning/ci_metrics.json` com resumo + trecho de logs + instruções para builder
  - Tracking de retries: lê/escreve `.planning/CI_RETRIES.json` (ou STATE.md) para limitar a max_retries (default 2)
...
Integração com Harness:
  - Harness lê exit code 2 e agenda `task("builder", context:{PLAN,SUMMARY,CI_REPORT})` com max 2 iterações.
...
def main():
    ...
    json_out = Path(args.json_out) if args.json_out else repo / ".planning" / "ci_metrics.json"
    md_out = Path(args.md_out) if args.md_out else repo / ".planning" / "CI_REPORT.md"
    ...
    if not dry_run:
        try:
            ...
            print(f"✅ CI_REPORT.md: {md_out} ({len(report_md)} chars)")
        except Exception as e:
            print(f"❌ Falha ao escrever CI_REPORT: {e}", file=sys.stderr)
        try:
            json_out.parent.mkdir(parents=True, exist_ok=True)
            json_out.write_text(json.dumps(metrics, indent=2, ensure_ascii=False), encoding="utf-8")
            print(f"✅ ci_metrics.json: {json_out}")
        except Exception as e:
            print(f"⚠️ Falha ao escrever ci_metrics.json: {e}")
    ...
    print(f"  report: {md_out if not dry_run else '(dry-run)'}")
    print("="*60)
